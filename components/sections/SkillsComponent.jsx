        ...
        {isMobile ? (
          ...
          >
            {skillsSections.map(({ key, title, description, skillList }) => (
              <AccordionItem key={key} value={key} className="border-none">
                <AccordionTrigger
                  className="w-full px-4 py-3 text-center font-bold rounded-md bg-[#232329] text-white hover:bg-accent/80 transition-all data-[state=open]:bg-accent data-[state=open]:text-white flex justify-between items-center"
                >
                  {title}
                </AccordionTrigger>
          ...
          >
            ...
            <div className="min-h-[70vh] w-full">
              {skillsSections.map(
                ({ key, title, description, skillList }) => (
